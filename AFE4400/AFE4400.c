...
unsigned long AFE44xx_Current_Register_Settings[36] = {
	CONTROL0_VAL,//+SPI_READ,           //Reg0:CONTROL0: CONTROL REGISTER 0
	...
	0x00000,                      //Reg31:?: ??
	(ENSEPGAIN + STAGE2EN_LED1 + STG2GAIN_LED1_12DB + CF_LED1_5P +  RF_LED1_100K),   //    0x00000,//              //Reg32:TIAGAIN: TRANS IMPEDANCE AMPLIFIER GAIN SETTING REGISTER
	0x00000A,// (AMBDAC_0uA + FLTRCNRSEL_500HZ + STAGE2EN_LED2 + STG2GAIN_LED2_12DB + CF_LED2_5P +  RF_LED2_100K),  //Reg33:TIA_AMB_GAIN: TRANS IMPEDANCE AAMPLIFIER AND AMBIENT CANELLATION STAGE GAIN
	(LEDCNTRL_VAL),                                                                                 //Reg34:LEDCNTRL: LED CONTROL REGISTER
	(TX_REF_1 + RST_CLK_ON_PD_ALM_PIN_DISABLE + ADC_BYP_DISABLE + TXBRGMOD_H_BRIDGE + DIGOUT_TRISTATE_DISABLE + XTAL_ENABLE + EN_FAST_DIAG + PDN_TX_OFF + PDN_RX_OFF + PDN_AFE_OFF)
	//Reg35:CONTROL2: CONTROL REGISTER 2 //bit 9
};
...
void Spi_Init(void) {
	...
	P4DIR |= BIT0 + BIT1 + BIT3;			// STE, SIMO and SCLK, as output
	P4DIR &= ~BIT2;                         	// SOMI as input
	P4OUT |= BIT0;								// Set STE high
	UCB1CTL1 |= UCSWRST;               	// Enable SW reset
	UCB1CTL0 |= UCMSB + UCCKPH + UCMST + UCSYNC;
...
{
	//port set..
	//Port 1.3 - AFE_RESETZ, P1.2 - AFE_PDNZ, P2.3 - ADC_RDY, P2.4- PD_ALM, P2.5 - LED_ALM,
	//P3.7 - DIAG_END
...
void AFE44xx_Default_Reg_Init(void)
{
	...
	AFE44x0_Reg_Write((unsigned char)TIAGAIN, AFE44xx_Current_Register_Settings[31]);            //0x20
...
void AFE44x0_Reg_Write(unsigned char reg_address, unsigned long data)
{
	...
	P4OUT &= ~0x01;   //  SENjh LOW FOR TRANSMISSION.
